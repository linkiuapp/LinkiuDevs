...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    @forelse($stores as $store)
        <div class="bg-white-50 rounded-lg p-0 overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-6">
                {{-- Header del card --}}
                ...
                <div class="mb-4">
                    @if($store->status === 'active')
                        <span class="bg-success-200 text-white-50 w-full text-center py-2 px-2 rounded text-xs font-medium block">Activa</span>
                    ...
                    @else
                        <span class="bg-error-200 text-white-50 w-full text-center py-2 px-2 rounded text-xs font-medium block">Suspendida</span>
                    @endif
                </div>
                ...
                <div class="flex gap-2">
                    <a href="{{ route('superlinkiu.stores.show', $store) }}"
                        class="flex-1 bg-primary-50 hover:bg-primary-200 hover:text-white-50 text-primary-300 p-2 rounded-lg text-center transition-colors">
                        <x-solar-eye-outline class="w-4 h-4 mx-auto" />
                    </a>
                    <a href="{{ route('superlinkiu.stores.edit', $store) }}"
                        class="flex-1 bg-info-50 hover:bg-info-200 hover:text-white-50 text-info-300 p-2 rounded-lg text-center transition-colors">
